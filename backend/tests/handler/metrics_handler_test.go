...
func (m *mockMetricsService) GetRegistrationMetrics(
	ctx context.Context,
	permissions []string,
	userID uuid.UUID,
) ([]models.MetricCard, error) {
	if slices.Contains(permissions, "View all appclients") {
		return []models.MetricCard{
			{Title: "Account Types", Value: "10"},
		}, nil
	}
	return []models.MetricCard{
		{Title: "Account Types", Value: "2"},
	}, nil
}
...
func TestGetRegistrationMetrics_Unauthorized(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	c.Request, _ = http.NewRequest(
		"GET",
		"/admin/metrics/registration",
		nil,
	)

	handler.GetRegistrationMetrics(c)
...
func TestGetRegistrationMetrics_Superadmin(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	if len(resp) != 1 || resp[0].Value != "10" {
		t.Errorf("unexpected response content: %v", resp)
	}
}

func TestGetRegistrationMetrics_Scoped(t *testing.T) {
	gin.SetMode(gin.TestMode)

    ...
    <Card className="border-0 ring-0 bg-transparent shadow-none overflow-visible">
      <CardContent className="space-y-6 text-center p-0">
        ...
        <form onSubmit={onVerify} className="space-y-5">
          <Input type="text" value={backupCode} onChange={(event) => onBackupCodeChange(event.target.value)} placeholder="Enter backup code" disabled={isVerifying} className="h-12 w-full rounded-xl border border-[#ffd700]/50 bg-white/95 px-4 text-center font-mono text-base font-semibold text-[#351018] outline-none transition placeholder:font-sans placeholder:text-sm placeholder:font-medium placeholder:text-slate-400 focus:border-[#ffd700] focus:ring-4 focus:ring-[#ffd700]/20 disabled:cursor-not-allowed disabled:opacity-60"/>

          <Button type="submit" disabled={isVerifying} className="h-12 w-full bg-[#991b1b] text-white hover:bg-[#7b0d15] font-bold transition duration-300">

...
const userPoolIconWrapClassName =
...
const userPoolTitleClassName = "text-2xl font-semibold tracking-tight text-[#7b0d15]";

const defaultDescriptionClassName = "text-sm text-gray-700";
const userPoolDescriptionClassName = "text-sm text-[#7d5c62]";
...
const userPoolSecondaryButtonClassName =
...
const userPoolPrimaryButtonClassName =
...
export default function DeleteConfirmModal({
  ...
  const isUserPoolVariant = variant === "userpool";

  return (
    <dialog
      className={`${
        isUserPoolVariant ? userPoolDialogClassName : defaultDialogClassName
      } ${open ? "modal-open" : ""}`}
    >
      <form
        method="dialog"
        className={isUserPoolVariant ? userPoolBoxClassName : defaultBoxClassName}
      >
        <div className="mb-6 flex justify-center">
          <div
            className={
              isUserPoolVariant
                ? userPoolIconWrapClassName
          ...
          >
            <svg
              ...
              fill="currentColor"
              className={`h-10 w-10 ${
                isUserPoolVariant ? "text-white" : "text-[#ffd700]"
              }`}
            >
              <path
                fillRule="evenodd"
        ...
        <h3 className={isUserPoolVariant ? userPoolTitleClassName : defaultTitleClassName}>
          {message}
        </h3>
        <p
          className={`mt-2 ${
            isUserPoolVariant
              ? userPoolDescriptionClassName
        ...
        <div className="modal-action justify-center gap-3">
          <button
            ...
            className={
              isUserPoolVariant
                ? userPoolSecondaryButtonClassName
          ...
          <button
            ...
            className={
              isUserPoolVariant
                ? userPoolPrimaryButtonClassName

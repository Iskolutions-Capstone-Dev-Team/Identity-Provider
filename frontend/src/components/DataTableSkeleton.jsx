...
const TABLE_THEMES = {
  ...
  userpool: {
    ...
    tableClassName: "table w-full min-w-[56rem]",
    headerRowClassName:
...
function renderCellContent(type, width, skeletonToneClassName) {
  if (type === "avatar") {
    return (
      <div className="flex justify-center">
        <div
          className={`skeleton h-10 w-10 rounded-full ${skeletonToneClassName}`}
  ...
  if (type === "stackedText") {
    return (
      <div className="flex flex-col items-center gap-2">
        <div
          className={`skeleton h-4 w-20 ${skeletonToneClassName}`}
          style={SKELETON_STYLE}
        />
        <div
          className={`skeleton h-4 w-28 ${skeletonToneClassName}`}
          style={SKELETON_STYLE}
        />
      </div>
    );
  ...
  if (type === "badge") {
    return (
      <div className="flex justify-center">
        <div
          className={`skeleton h-6 rounded-full ${width || "w-16"} ${skeletonToneClassName}`}
          style={SKELETON_STYLE}
        />
      </div>
    );
  ...
  if (type === "badges") {
    return (
      <div className="flex justify-center gap-2">
        <div
          className={`skeleton h-6 w-16 rounded-full ${skeletonToneClassName}`}
          style={SKELETON_STYLE}
        />
        <div
          className={`skeleton h-6 w-12 rounded-full ${skeletonToneClassName}`}
          style={SKELETON_STYLE}
        />
      </div>
    );
  ...
  if (type === "button") {
    return (
      <div className="flex justify-center">
        <div
          className={`skeleton h-9 w-16 rounded-lg ${skeletonToneClassName}`}
          style={SKELETON_STYLE}
        />
      </div>
    );
  ...
  if (type === "iconButton") {
    return (
      <div className="flex justify-center">
        <div
          className={`skeleton h-10 w-10 rounded-xl ${skeletonToneClassName}`}
          style={SKELETON_STYLE}
        />
      </div>
    );
  ...
  return (
    <div
      className={`mx-auto skeleton h-4 ${width || DEFAULT_TEXT_WIDTH} ${skeletonToneClassName}`}
      style={SKELETON_STYLE}
    />
  );
}
              ...
              <tr key={rowIndex}>
                {columns.map((column) => (
                  <td
                    ...
                    className={styles.bodyCellClassName}
                  >
                    {renderCellContent(
                      column.type,
                      column.width,

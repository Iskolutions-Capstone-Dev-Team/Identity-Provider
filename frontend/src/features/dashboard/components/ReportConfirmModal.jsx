...
export default function ReportConfirmModal({ open, colorMode = "light", isGenerating = false, onCancel, onConfirm }) {
  ...
  const iconWrapClassName = isDarkMode
    ? "flex h-20 w-20 items-center justify-center rounded-[1.5rem] border border-[#f8d24e]/25 bg-[radial-gradient(circle_at_top,rgba(248,210,78,0.16),transparent_45%),linear-gradient(135deg,#7b0d15_0%,#263345_60%,#1a121c_100%)] text-white shadow-[0_24px_50px_-28px_rgba(2,6,23,0.8)]"
  ...
  const primaryButtonClassName = isDarkMode
    ? "btn h-12 rounded-[1rem] border border-[#f8d24e]/35 bg-[linear-gradient(135deg,#7b0d15_0%,#4f1018_100%)] px-6 text-white transition hover:border-[#f8d24e] hover:bg-[#8f121b] disabled:opacity-40 disabled:border-transparent disabled:bg-[#4f1018]/50 disabled:text-white/50"
    : "btn h-12 rounded-[1rem] border border-[#7b0d15] bg-[#7b0d15] px-6 text-white transition hover:border-[#5a0b12] hover:bg-[#5a0b12] disabled:opacity-50 disabled:border-transparent disabled:bg-[#7b0d15]/50 disabled:text-white/70";
  ...
  const handleConfirm = () => {
    onConfirm({
      includeSecurityAnalysis: selectedFilters.includes("includeSecurityAnalysis"),
      includeAuthStats: selectedFilters.includes("includeAuthStats"),
      includeFailedAttempts: selectedFilters.includes("includeFailedAttempts"),
    });
  };
  ...
  return (
    ...
    >
      <form method="dialog" className={boxClassName} style={{ overflow: 'visible' }}>
        ...
        <p className={`mt-2 ${descriptionClassName}`}>
          Generate and download the latest authentication metrics and security report. Select the sections to include:
        </p>

        <div className="mt-6 flex flex-col gap-3 px-4 text-left">
          <ReportFilterSelect
        ...
        <div className="modal-action justify-center gap-3 mt-8">
          <button type="button" className={secondaryButtonClassName} onClick={onCancel} disabled={isGenerating}>
            Cancel
          </button>
          <button type="button" className={primaryButtonClassName} onClick={handleConfirm} disabled={isGenerating || selectedFilters.length === 0}>

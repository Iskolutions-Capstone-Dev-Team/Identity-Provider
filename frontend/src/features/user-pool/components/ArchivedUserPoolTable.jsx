...
export default function ArchivedUserPoolTable({
  loading = false,
  users = [],
  userType = "regular",
  appClients = [],
  ...
  const showActionsColumn = showViewAction || showUnarchiveAction || showHardDeleteAction;
          ...
          {users.map((user) => {
            ...
            return (
              <TableRow key={user.id}>
                ...
                <TableCell className="text-center">
                  <Badge 
                    variant={user.status === "deleted" ? "secondary" : getStatusBadgeVariant(user.status)} 
                    className={`capitalize font-semibold rounded-full px-3 ${
                      user.status === "deleted" 
                        ? "bg-red-50 text-red-700 hover:bg-red-50 dark:bg-red-950 dark:text-red-300 dark:hover:bg-red-950" 
                        : ""
                    }`}
                  >
                    {user.status}

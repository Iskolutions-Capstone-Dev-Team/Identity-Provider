  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 w-full">
      {clients.map((client) => {
        ...
        return (
          <Card key={client.clientId || client.id} className="overflow-hidden border shadow-sm flex flex-col hover:border-[#7b0d15]/40 dark:hover:border-[#f8d24e]/40 transition-colors">
            ...
            <CardFooter className="p-4 pt-3 flex items-center justify-between border-t mt-auto bg-card">
              ...
              <div className="flex items-center justify-center gap-1">
                      ...
                      {showDeleteAction && (
                        <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-[#7b0d15] hover:text-[#ffd21a] dark:hover:bg-muted dark:hover:text-foreground transition-colors" onClick={() => onDelete?.(client)} title="Delete">
